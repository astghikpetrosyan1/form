...
export class RadioGroup extends React.Component<RadioGroupProps, RadioGroupState> {
  ...
  render(): JSX.Element {
    ...
    const inputFields: Array<JSX.Element> = options.map((e: Options, i: number) => {
      ...
      return (
        <div key={inputId} className="choice-image-card">
          { showChoiceImage ? (
            <div>
              { !image ? (
                <img src={NoImage} alt='' width="223px" height="200px" style={{ objectFit: 'contain' }} />
              ) : fileType(image) === 'image' ? (
                <img src={image || NoImage} alt='' width="223px" height="200px" style={{ objectFit: 'contain' }} />
          ...
          <div className="checkbox-container atom_radio">
            <input
              id={inputId}
    ...
    const content: JSX.Element = (
      <React.Fragment>
        <PrivateRadioGroup name={id} value={selected} classNameGroup={`${this.props.classNameGroup} group-container`}>
